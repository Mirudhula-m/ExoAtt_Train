...
function animateBall(){
    document.getElementById("info").style.visibility = 'hidden'; // added
...
*/
            ...
            $('#info-h').text("Estimated viewing distance (cm): "+data["viewDistance_mm"]/10); //changed


            // my_codes
            if(data["viewDistance_mm"]/10 < 46.5)
            {
                $('#info-p').text("You are too close to the screen. Please shift a your seat away from the screen and try again. Alternatively, please measure 50 cm in between yourself and the screen, if possible, and start the test again.");
                $('#click').text(Math.max(5, 0));
                $("#start").attr("disabled", false);    
            }
            if(data["viewDistance_mm"]/10 > 53.5)
            {
                $('#info-p').text("You are too away from the screen. Please shift a your seat closer to the screen and try again. Alternatively, please measure 50 cm in between yourself and the screen, if possible, and start the test again.");
                $('#click').text(Math.max(5, 0));
                $("#start").attr("disabled", false); 
            }
            if(data["viewDistance_mm"]/10 >= 46.5 && data["viewDistance_mm"]/10 <= 53.5)
            {
                $('#info-p').text("Press Enter to continue");
            }
